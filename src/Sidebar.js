  ...
  render() {
    if (this.props.show) {
      ...
      if (area.length > 0 || purpose.length > 0) {
        if (area.length > 0) {
          area = 'Ámbitos: #' + area.map(el => el.replace(/\s/g, '')).join(' #')
        }
        else {
          area = ''
        }
        if (purpose.length > 0) {
          purpose = 'Temáticas: #' + purpose.map(el => el.replace(/\s/g, '')).join(' #')
        }
        else {
          purpose = ''
        }
        tagged = <span>Esta iniciativa contiene los tags:<br/>{area}<br/>{purpose} </span> 
      }
      else {
